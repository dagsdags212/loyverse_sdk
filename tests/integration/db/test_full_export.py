...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_upsert_behavior(temp_db):
    """Test that INSERT OR REPLACE provides upsert semantics."""
    client = LoyverseClient(api_token="fake_token_for_testing")
    client.init_duckdb_schema(temp_db)
    ...
    conn.execute("""
        INSERT INTO categories (id, name, color, created_at, updated_at)
        VALUES ('cat1', 'Original Name', 'RED', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    """)
    ...
    conn.execute("""
        ...
        VALUES ('cat1', 'Updated Name', 'BLUE', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    """)
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_database_indexes_creation(temp_db):
    """Test that indexes are created successfully."""
    client = LoyverseClient(api_token="fake_token_for_testing")
    client.init_duckdb_schema(temp_db)

    from loyverse_sdk.db.schema_builder import create_indexes
