...
class TestUpsertBehavior:
    """Test INSERT OR REPLACE (upsert) behavior."""

    def test_insert_or_replace_updates_existing_record(self, db_with_schema):
        ...
        conn.execute("""
            INSERT INTO categories (id, name, color, created_at, updated_at)
            VALUES ('cat1', 'Original', 'RED', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """)
        ...
        conn.execute("""
            INSERT OR REPLACE INTO categories (id, name, color, created_at, updated_at)
            VALUES ('cat1', 'Updated', 'BLUE', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """)
...
class TestSyncMetadata:
    """Test sync_metadata table functionality."""

    def test_sync_metadata_insert(self, db_with_schema):
        """Test inserting sync metadata."""
        conn = duckdb.connect(db_with_schema)

        conn.execute("""
            INSERT INTO sync_metadata (resource_name, last_sync_at, records_count, sync_type)
            VALUES ('receipts', CURRENT_TIMESTAMP, 1000, 'full')
        """)

        # Verify insert
        result = conn.execute("""
            SELECT resource_name, records_count, sync_type
            FROM sync_metadata
            WHERE resource_name = 'receipts'
        """).fetchone()
    ...
    def test_sync_metadata_upsert(self, db_with_schema):
        """Test upserting sync metadata."""
        conn = duckdb.connect(db_with_schema)
        ...
        # Upsert
        conn.execute("""
            INSERT OR REPLACE INTO sync_metadata (resource_name, last_sync_at, records_count, sync_type)
            VALUES ('receipts', CURRENT_TIMESTAMP, 1500, 'full')
        """)
